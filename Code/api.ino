...
void HandleFileDownload(AsyncWebServerRequest *r) {
  ...
  if (preview == "true") {
    // Serve
    String filePath = "/www" + targetFile;
    String mimeType = getMime(filePath);

    if (SD.exists(filePath.c_str())) {
      //server.serveStatic(filePath.c_str(), SD, filePath.c_str(), mimeType.c_str());
      r->send(SD, filePath.c_str(), "application/octet-stream", true);
    }
  } else {
    // Download
    String filePath = "/www" + targetFile;
    ...
    if (SD.exists(filePath.c_str())) {
      //server.serveStatic(filePath.c_str(), SD, filePath.c_str(), "application/octet-stream");
      r->send(SD, filePath.c_str(), "application/octet-stream", true);
    }
  }
}
...
/* Handle Load and Set of Preference Value */
