...
// Serve directory entries from SD card
void HandleSDContent(AsyncWebServerRequest *request, String dirName, String dirToList) {
  AsyncResponseStream *response = request->beginResponseStream("text/html");

  // Open the directory in the SD card
  File directory = SD.open(dirToList);

  // Check if the directory is open
  if (!directory) {
    SendErrorResp(request, "Unable to open directory");
    return;
  }
  ...
  // List the contents of the directory from the SD card
  while (true) {
  ...
  // Close the directory in the SD card
  directory.close();
...
void initWebServer() {
  ...
  */
    server.on("/sdcontent", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (IsUserAuthed(request)) {
      HandleSDContent(request, "SD Card", "/");  // Modify the path as needed
    } else {
