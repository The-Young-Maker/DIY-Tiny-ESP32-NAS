...
void scanSDCardForKeyword(const String& directoryPath, const String& keyword, int* matchCounter, AsyncResponseStream* response) {
  String modifiedPath = directoryPath;
  if (modifiedPath.length() > 4 && modifiedPath.substring(0, 4) == "/www" && modifiedPath.length() > 4 && modifiedPath[4] != '/') {
    modifiedPath = modifiedPath.substring(0, 4) + '/' + modifiedPath.substring(4);
  }
  ...
  File directory = SD.open(modifiedPath);
  if (!directory) {
    Serial.println("Error opening directory " + modifiedPath);
    return;
  }
